...
import Detailsdalog from '../../../components/Detailsdalog.vue'
export default {
  name: "VideoManage",
  components: {
    Detailsdalog
  ...
  methods: {
    ...
    infoAdd() {
      ...
      this.$refs.detailsdalog.setDialogForm({});
      this.$refs.detailsdalog.setdatafrom(this.$refs.detailsdalog.dialogForm, 'status', 4);
    },
    // 关闭弹出窗口
    formClose(bool) {
      this.dialogType = '';
    ...
    handleDetail(item) {
      ...
      this.$refs.detailsdalog.setDialogForm(JSON.parse(JSON.stringify(item)));
      this.dialogShow = true;
    },
    // 修改
    handleUpdate(item) {
    ...
    uploadImage(params) {
      ...
      this.$refs.detailsdalog.setdatafrom(this.$refs.detailsdalog.dialogForm, 'resourceimage', contents.getunitcos().imgdata.message);
      //   } else {
    ...
    uploadImage1(params) {
      ...
      //   if (_result.code === '0000' && _result.innercode === '0000') {
      _self.$message.success('上传成功');
      this.$refs.detailsdalog.setdatafrom(this.$refs.detailsdalog.dialogForm, 'resourceipicture', contents.getunitcos().imgdata.message);
      //   } else {
      //     _self.$message('上传失败,请重新上传')
      //   }
      // })
    ...
    uploadVideo(params) {
      const _self = this;
      _self.$message.success('上传成功');
      this.$refs.detailsdalog.setdatafrom(this.$refs.detailsdalog.dialogForm, 'resourceviedo', contents.getunitcos().video.message);
    },
    // 推送资源
    sendData(item) {
      const _self = this;
